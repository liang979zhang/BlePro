...
public class DashboardView2 extends View {
    ...
    private static int innerR = (int) (200 * 0.95f);//内部圆环的半径
    ...
    private void initPaint() {
        ...
        arcRectF = new RectF(-outerR, -outerR, outerR, outerR);
        arcInnerRectF = new RectF(-innerR, -innerR, innerR, innerR);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSize < DensityUtils.dp2px(getContext(), minWidthDP) || heightSize < DensityUtils.dp2px(getContext(), minHeightDP)) {
            widthSize = DensityUtils.dp2px(getContext(), minWidthDP);
            heightSize = DensityUtils.dp2px(getContext(), minHeightDP);
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        widthDash = DensityUtils.dp2px(getContext(), 50);
    ...
    protected void onDraw(Canvas canvas) {
        arcPaint.setColor(arcColor);
        canvas.drawArc(arcRectF, 180, 90, true, arcPaint);  //绘制外弧形
        canvas.drawArc(arcInnerRectF, 180, 90, true, arcInnerPaint);  //绘制内部弧形
        canvas.save();
//        drawScale(canvas);  //绘制刻度
//        drawScale2(canvas);  //绘制刻度
        canvas.restore();
//        drawText(canvas);  //绘制文本
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            int jinu = (int) (degree / Math.PI * 180);
            if (isChoosePointer) {
//                Log.e("aaa", "x====" + x);
//                Log.e("aaa", "y====" + y);
                if (y <= mheight && x != mwidth / 2 && jinu <= 90) {
